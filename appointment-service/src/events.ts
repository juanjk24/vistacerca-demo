...
export const STATUSES = ['PENDING', 'CONFIRMED', 'ATTENDED', 'RESCHEDULED', 'NO_SHOW'] as const;
...
export async function connectEvents(retries = 20): Promise<void> {
  for (let i = 1; i <= retries; i++) {
    try {
      ...
      console.log('[appointment-service] publicador RabbitMQ listo');
      return;
    } catch {
      console.log(`[appointment-service] esperando RabbitMQ (${i}/${retries})...`);
...
export async function publishEvent(key: string, payload: unknown): Promise<void> {
  ...
  console.log(`[appointment-service] publicado evento ${key}`);
}